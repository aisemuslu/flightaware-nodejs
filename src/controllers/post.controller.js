 ...
 *
 * @apiParam {Int} skip Number of skip posts, - query 
 * @apiParam {Int} limit Maximum number of posts, - query 
 *
 ...
 *
 * @apiParam {String} pass Only logged in users can do this.
 *
 * @apiHeaderExample {json} Header-Example:
 * {
 *  "AUTHORIZATION": "JWT eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI1OTBhMWI3ODAzMDI3N2NiNjQxM2JhZGUiLCJpYXQiOjE0OTM4MzQ2MTZ9.RSlMF6RRwAALZQRdfKrOZWnuHBk-mQNnRcCLJsc8zio"
 ...
 *    HTTP/1.1 401 Unauthorized
 */
async function favoritePost(req, res, next) {
  try {
...
async function favoritePostx(req, res, next) {
  try {
    const user = await User.findById(req.user._id);
    await user._favorites.posts(req.params.id);
    return res.sendStatus(HTTPStatus.OK);
...
module.exports={favoritePost, updatePost, deletePost, validation, getList, getById, create, favoritePostx}
